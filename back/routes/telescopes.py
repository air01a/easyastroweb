...
from services.configurator import save_telescope_config, delete_telescope_config, get_telescope_config, get_telescope_config_schema, set_default_telescope_config, CONFIG, TELESCOPE_PATH, TELESCOPE_SCHEMA_PATH
...
@router.post("/")
async def api_set_telescopes(payload: ConfigPayload):
...
async def api_delete_observatories(payload: ConfigPayload):
    """Set the list of telescopes."""
    (error, error_str) = await delete_telescope_config(TELESCOPE_PATH, payload, 'telescope')
    if not error:
        raise HTTPException(status_code=500, detail=error_str)
